...
const runtimeValidation = await stage('M5-runtime-validation', async () => {
  ...
  const grep = `^(${testNames.join('|')})#!/usr/bin/env node
import { execFileSync } from 'node:child_process';
import { readFile, writeFile } from 'node:fs/promises';
import path from 'node:path';
import { pathToFileURL } from 'node:url';
...
;
  ...
  const validation = packetApi.coverageValidationResult({
    ...
    observed: {
      ...
      sourceRevision,
      sourcePath,
      sourceBlobSha: command('git', ['rev-parse', `${sourceRevision}:${sourcePath}`], targetRoot),
      testPath,
      testBlobSha: command('git', ['rev-parse', `${sourceRevision}:${testPath}`], targetRoot),
      project: 'mobile-small',
      testNames,
      witnesses: [
